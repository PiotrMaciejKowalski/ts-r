...
dane_p <- read.csv("dane_EUR_PLN.csv") %>%
    mutate(Data = as.Date(Data)) %>%
    complete(Data = seq.Date(min(Data), 
                                 max(Data), 
                                 by = "day"))

dane <- dane_p[dane_p$Data >= data_start, c("Data", typ)]
head(dane)
```
...
słabej stacjonarności naszego szeregu.
...
```
<!-- Przeprowadźmy zatem test aby sprawdzić cechy słabej stacjonarności szeregu  -->
<!-- zróżnicowanego. -->
...
<!-- Zróżnicowany szereg wykazuje cechy słabej stajonarności. Zatem nasze modele  -->
<!-- predykcyjne stworzymy w oparciu o szereg zróżnicowany. Wprowadzimy zatem parametr  -->
<!-- `d`, który będzie odpowiadał za zróżnicowanie szerego.  -->
...
<!-- Narysujmy ponownie wykres (za pomocą `stl()`) z którego wprost odczytamy oryginalne  -->
<!-- dane, trend, sezonowość i resztę. -->

```{r include = FALSE}
nsc_stl <- stl(nsc, s.window = "periodic")
plot(nsc_stl, col = brewer.pal(n = 4, name = "PRGn")[1])
```
...
```{r}
pkurs <- rep(NULL,4)
for (i in 1:4) { pkurs[i] <- predykcje[[i]]$mean[7]}
pkurs
```
