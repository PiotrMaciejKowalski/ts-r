...
## Filtrowanie potokowe

Przeprowadzimy teraz 3 procesy filtrowania potokowego, a ich wyniki zaprezentujemy na wykresach. Dane będziemy filtrować ze względu na różne przedziały wartości poszczególnych kursów lub ze względu na wartość ich różnicy.

Najpierw rozważmy pomiary dla kursów szczytu i dołka powyżej wartości 6.

```{r filtrowanie potokowe 1}
library(dplyr)
wybrane_dane %>%
    filter( szczyt >= 6, dołek >= 6) %>%
    mutate(`Średnia ze szczytu i dołka` = (szczyt + dołek)/2) %>%
    group_by(godzina) %>%
    summarise(średnia = mean(`Średnia ze szczytu i dołka`, na.rm = T))-> result1
head(result1)
x <- c(8,12,16,20)
plot(x, result1$średnia, xlab = "Godzina", main = "Średnia z kursów szczytu i dołka dla wybranych wartości", ylab="", type ="l", col ="blue", lwd =3)
```

Teraz przyjrzyjmy się naszym danym dla kursu otwarcia <= 6.1 oraz kursu zamknięcia >=6.

```{r filtrowanie potokowe 2}
wybrane_dane %>%
    filter( `kurs otwarcia` <= 6.1, `kurs zamknięcia` >= 6) %>%
    mutate(`Różnica kursów otwarcia i zamknięcia` = abs(`kurs otwarcia` - `kurs zamknięcia`), `Różnica duża` = `Różnica kursów otwarcia i zamknięcia` > 0.001) -> result2
head(result2)
plot(result2$czas, result2$`Różnica kursów otwarcia i zamknięcia`, xlab = "Data pomiaru", main = "Różnica kursów otwarcia i zamknięcia dla wybranych wartości", ylab="", type ="l", col ="blue")
```

Ostatnią analizę wykorzystująca filtrowanie potokowe skonstruujemy na podstawie danych, dla których moduł różnicy kursów otwarcia i zamknięcia nie przekracza wartości 0.0002.

```{r filtrowanie potokowe 3}
wybrane_dane %>%
    filter( abs(`kurs otwarcia`-`kurs zamknięcia`) <= 0.0002) -> result3
head(result3)
plot(result3$czas, abs(result3$`kurs otwarcia`-result3$`kurs zamknięcia`), xlab = "Data pomiaru", main = "Różnica kursów otwarcia i zamknięcia dla tylko małych wartości", ylab="", type ="l", col ="blue")
```

Zauważmy, że do powyższych analiz skorzystaliśmy: 
3x z funkcji filter(),
2x z funkcji mutate(),
1x z funkcji group_by(),
1x z funkcji summarise().
