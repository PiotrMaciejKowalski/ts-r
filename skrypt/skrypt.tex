...
\section{Algorytm Bellmana}

Równania Bellman pojawiły sie w naszej teorii w chwili gdy zaczęliśmy zastanawiać się nad możliwością odszukania optymalnej polityki...

%TODO to be continued...

%\begin{tabular}{|c|c|}\hline
%Polityka optymalna & $ \pi^\ast = \argmax_{\pi} \Ex{ \sum_{n=0}^{\infty} \lambda^n R(s_n)}{\pi} $ \\\hline
%\uncover<2->{Użyteczność polityki} & \uncover<2->{$ U^\pi(s) = \Ex{\sum_{n=0}^{\infty} \lambda^n R(s_n)}{\pi, s_0 = s} $} \\\hline 
%\uncover<3->{Właściwa użyteczność} & \uncover<3-> {$U(s) = U^{\pi^\ast}(s)$}\\\hline
%\end{tabular}
%
%\uncover<4->{
%\begin{block}{Polityka optymalna}
%\begin{equation*}
%\pi^\ast (s) = \argmax_{a} \sum_{s'} T(s,a,s') U(s').
%\end{equation*}
%\end{block}
%}
%\uncover<5->{
%\begin{block}{\uncover<6->{Równanie Bellmanna}}
%\begin{equation}
%U(s) = R(s) + \lambda \max_{a} \sum_{s'} T(s,a,s') U(s')
%\end{equation}
%\end{block}
