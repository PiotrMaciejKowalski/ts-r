---
title: "Predykcja kursu euro do funta brytyjskiego"
...
## Informacje ogĂłlne
Raport dotyczy analizy ceny kursu sprzedaĹĽy euro do funta brytyjskiego.  
Dane pochodzÄ… ze strony histdata.com.  
  
Okres z ktĂłrego pochodzÄ… dane: 01.01.2019-08.01.2019r.
...
```
Charakterystyka kursu zamkniÄ™cia
```{r}
summary(d$zamkniecie)
```

PoniĹĽsze wykresy zostaĹ‚y sporzÄ…dzone w oparciu o kurs zamkniÄ™cia.\newline
\newline
Ĺšredni kurs dla wszystkich danych wynosi 0,8768, natomiast mediana dla wszystkich danych wynosi 0,8760.

## Wykresy


```{r pressure, echo=FALSE}
plot(1:nrow(d),d$zamkniecie,type = "l",xlab = "Pomiary",ylab = "Kurs",main = "Kursy dla wszystkich pomiarĂłw",col="black")
abline(h=mean(d$zamkniecie),lty=1, col="blue")
abline(h=median(d$zamkniecie),lty=1, col="deeppink2")
legend("topright", legend=c("Mediana", "Ĺšredni kurs"),
...
hist(d$zamkniecie, 
     main="Histogram dla danych opisujÄ…cych kurs zamkniÄ™cia", 
     xlab="Kursy", 
     border="blue", 
     col="green")
```
...
legend("topright", legend=c("Mediana", "Ĺšredni kurs","Mediana dla 8:00","Sredni kurs dla 8:00"),col=c("deeppink2","blue","lightpink","deepskyblue"), lty=c(1,1,2,2), cex=0.8,bg='lightgrey')
```
...
legend("topright", legend=c("Mediana", "Ĺšredni kurs","Mediana dla 12:00","Sredni kurs dla 12:00"),col=c("deeppink2","blue","lightpink","deepskyblue"), lty=c(1,1,2,2), cex=0.8,bg='lightgrey')
```
...
legend("topright", legend=c("Mediana", "Ĺšredni kurs","Mediana dla 16:00","Sredni kurs dla 16:00"),col=c("deeppink2","blue","lightpink","deepskyblue"), lty=c(1,1,2,2), cex=0.8,bg='lightgrey')
```
...
legend("topright", legend=c("Mediana", "Ĺšredni kurs","Mediana dla 20:00","Sredni kurs dla 20:00"),col=c("deeppink2","blue","lightpink","deepskyblue"), lty=c(1,1,2,2), cex=0.8,bg='lightgrey')
...
Do predykcji danych zastosujemy regresję liniową, gdzie zmienną objaśnianą jest kurs, a objaśniająca numer pomiaru. 
...
Teraz zajmiemy się przewidywaniem pozostałych wartości, według powyższego modelu.
...
Wartość tej statystyki jest mnijesza niz 1, zatem widać, ża nasza prognoza jest skuteczniejsza niż prognoza naiwna. 
...
legend("topleft", legend=c("10 pomiarów", "20 pomiarów","30 pomiarów"),col=c("red","blue","pink"), lty=c(1,1,1), cex=0.8,bg='lightgrey')
b1<-coef(reg1)[1]
a1<-coef(reg1)[2]
b2<-coef(reg2)[1]
a2<-coef(reg2)[2]
...
abline(reg3,col="pink")
legend("topleft", legend=c("10 pomiarów", "20 pomiarów","30 pomiarów","wszystkie pomiary"),col=c("red","blue","pink","black"), lty=c(1,1,1,1), cex=0.8,bg='lightgrey')
...
Błąd średniokwadratowy, zgodnie z poprzednim stwierdzeniem, jest najmnijeszy dla zbioru uczącego składającego się z 20 pomiarów. Należy zatem wybrać do naszej prognozy właśnie ten model. 