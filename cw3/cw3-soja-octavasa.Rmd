...
Wykonamy prognozę modelem regresji liniowej dla wszystkich danych.
 Nasze dane podzieliłam na dwa zbióry, jeden dla 11 ostatnich wartości a drugi dla reszty
...
prog2<- predict(reg2)
prog1<- predict(reg1)
plot(ramka1$Data2,prog1)
plot(prog2)
plot(ramka2,col="blue", ylab="Kurs zamkniecia", xlab="Data")
lines(ramka2$Data2, prog2, type = "l", col="red")
```

Wyznaczmy błąd średniokwadratowy
```{r}
mean((prog2-ramka2$Zamkniecie)^2)
```
oraz wartość statystki Theil'a

```{r}
sum((prog2-ramka2$Zamkniecie)^2)/sum(ramka2$Zamkniecie^2)
```
