...
data<-dane$Data
otwarcie<-dane$Otwarcie
dzien<-as.numeric(1:length(data))
d<-data.frame(dzien,data,otwarcie)
head(d)
...
```{r}
zb_uczacy<-data.frame(dzien=dzien[1:45],data=data[1:45],otwarcie=otwarcie[1:45])
...
```{r}
model<-lm(otwarcie~dzien)
summary(model)
plot(dzien,  otwarcie,type="l",pch=20, cex=1,xlab = "Data", ylab = "Cena otwarcia akcji")
...
```{r}
prognoza<-predict(model, newdata = zb_testowy)
prognoza
plot(zb_testowy$dzien,zb_testowy$otwarcie,xlab = "Data",ylab = "Cena otwarcia akcji", main="Model z trendem liniowym dla wszytskich przeszłych danych", type="l")
lines(prognoza)
```
...
```{r}
liczba_dni<-nrow(dane)
model_n<-lm(otwarcie~dzien,data=zb_uczacy[(liczba_dni+1-20):liczba_dni,])
summary(model_n)
plot(dzien,  otwarcie,type="l",xlab = "Data", ylab = "Cena otwarcia akcji")
...
```{r}
prognoza1=predict(model_n, newdata = zb_testowy)
plot(zb_testowy$dzien, zb_testowy$otwarcie, xlab = "Data", ylab="Ceny otwarcia spółki AWM", type="l", main="Prognoza ceny zamknięcia")
lines(prognoza1, type="l", lwd = "2")
```
...
```{r}
b_sredkw<-mean(((prognoza1 - zb_testowy$otwarcie)^2))
b_sredkw
stat_Th<-sum(((prognoza1-zb_testowy$otwarcie)^2))/sum((zb_testowy$otwarcie^2))
stat_Th
```
