...
chwila <- 60
stala <-  30
uczacy <- cena_otw[(chwila-stala):(chwila-1),]
...
```{r}
czas2 <- as.numeric(cena_otw$Data_co)
regresja2 <- lm(cena_otw$cenaa ~ czas2)
##summary(regresja2)
...
Jak widac model jest słabo dopasowany.Rysujemy wykres prognozy dla zbioru testowego.
```{r}
plot(testowy$Data_co, testowy$cenaa, "p", col = "brown", main = "Prognoza")
t<-as.numeric(testowy$Data_co) 
lines(t, prognoza1)
```
