...
```{r}
p=70
zbiorU=data.frame(cbind(Data2=Data2[1:(p-1)],Otwarcie=Otwarcie[1:(p-1)]))
zbiorT=data.frame(cbind(Data2=Data2[p:length(Data2)],Otwarcie=Otwarcie[p:length(Otwarcie)]))
```

```{r}
plot(zbiorU$Data2, zbiorU$Otwarcie,'p',col="red", main = "Predykcja na zbiorze uczącym", xlab="Dni", ylab="Otwarcie")
```

```{r}
model <- lm(zbiorU$Otwarcie ~ zbiorU$Data2)
summary(model)
```

```{r}
plot(zbiorU$Data2, zbiorU$Otwarcie,'p',col="red");
abline(model)
```
...
predykcja = a * zbiorT$Data2 + b
plot(zbiorT$Data2, zbiorT$Otwarcie, main = "Predykcja dla modelu ", xlab="Dni", ylab="Kursy")
lines(zbiorT$Data2, predykcja, col= "blue")
```

```{r}
bladsr<- mean((predykcja-zbiorT$Otwarcie)^2)
theila <- sum((predykcja-zbiorT$Otwarcie)^2)/sum(zbiorT$Otwarcie^2)
```
```{r}
bladsr
theila
