...
reg <- lm(szczyt ~ `numer dnia`, ramka_do_modelu[1:(ilosc_dni - dnie_do_prognozy), ])
summary(reg)
plot(ramka_do_modelu$`numer dnia`[1:(ilosc_dni - dnie_do_prognozy)], ramka_do_modelu$szczyt[1:(ilosc_dni - dnie_do_prognozy)], xlab = "Numer dnia z archiwum", ylab = "Wartość kursu szczytu")
abline(reg, col = "red")
```
...
a <- reg$coeff[2]
b <- reg$coeff[1]
plot(ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni], ramka_do_modelu$szczyt[(ilosc_dni - dnie_do_prognozy):ilosc_dni], ylim = c(5.7, 6.2), xlab = "Numer dnia z archiwum", ylab = "Wartość kursu szczytu")
lines(ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni], a*ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni]+b, col = "red")
```
...
sredni_blad_kwadratowy <-function(coeff1, coeff2)
    {
        return(mean((ramka_do_modelu$szczyt[(ilosc_dni - dnie_do_prognozy):ilosc_dni]- coeff1*ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni]-coeff2)^2))
    }
statystyka_Theila <- function(coeff1, coeff2)
    {
        return(sum((ramka_do_modelu$szczyt[(ilosc_dni - dnie_do_prognozy-1):(ilosc_dni-1)]- coeff1*ramka_do_modelu$`numer dnia`[(ilosc_dni -dnie_do_prognozy):ilosc_dni]-coeff2)^2)/sum((ramka_do_modelu$szczyt[(ilosc_dni - dnie_do_prognozy-1):(ilosc_dni-1)])^2))
    }
```
...
n <- 10
reg_n <- lm(szczyt ~ `numer dnia`, ramka_do_modelu[(ilosc_dni- n - dnie_do_prognozy+1):(ilosc_dni - dnie_do_prognozy), ])
summary(reg_n)
plot(ramka_do_modelu$`numer dnia`[(ilosc_dni- n - dnie_do_prognozy+1):(ilosc_dni - dnie_do_prognozy)], ramka_do_modelu$szczyt[(ilosc_dni- n - dnie_do_prognozy+1):(ilosc_dni - dnie_do_prognozy)], xlab = "Numer dnia z archiwum", ylab = "Wartość kursu szczytu")
abline(reg_n, col = "red")
```

Możemy zauważyć, że ogólna tendencja jest chyba w tym przypadku oddana nieco gorzej niż poprzednio (co widać nawet po wskaźniku R-squared, który jest tu wyższy). Przejdźmy jednak do predykcji i zobaczmy, co tu się wydarzy. 

```{r predykcja_model_2}
c <- reg_n$coeff[2]
d <- reg_n$coeff[1]
plot(ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni], ramka_do_modelu$szczyt[(ilosc_dni - dnie_do_prognozy):ilosc_dni], xlab = "Numer dnia z archiwum", ylab = "Wartość kursu szczytu")
lines(ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni], c*ramka_do_modelu$`numer dnia`[(ilosc_dni - dnie_do_prognozy):ilosc_dni]+d, col = "red")
```
Jak widać pomimo, wydawałoby się mniej dopasowanego modelu niż poprzednio, otrzymaliśmy całkiem przyzwoitą predykcję na ostatnie 8 dni, a na pewno dużo lepszą niż poprzednia. Gdy przyjrzymy się dłużej poprzednim wykresom, możemy dojść do wniosku, że jest to skutkiem "przestarzałych" danych, z których korzystał wcześniejszy model. Łatwo zauważyć, że ogólna tendencja od początku strycznia do marca jest malejąca, jednak gdy przyjrzymy się kursom szczytu pod koniec lutego, to widać, że wartości rosną. Sprawdźmy jak wyglądają kolejno: średni błąd kwadratowy oraz statystyka Theil'a (porównanie z prognozą naiwną).
